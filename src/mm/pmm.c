...
/*
    ...
    new plan just use 1 byte per page, so you need one page full of page
   metadata
 */
#define PGSIZE 4096
#define bookkeep_end ((uintptr_t)HEAP_START + (PGSIZE))

typedef enum { BK_FREE = 0, BK_TAKEN = 1, BK_RESERVED = 2 } bk_state_t;
...
void *alloc_page(size_t num_pages) {
  ...
  */
  ...
  kassert((uintptr_t)HEAP_START < 0x400000,
          "heap must be < 4MiB for current paging init");
  // start at first page instead of page 0 because the first page is reserved
  // for bookkeeping

  kassert((uintptr_t)address % 4096 == 0,
          "address is misaligned not a multiple of 4096");
...
void free_page(void *address) {
  ...
  if ((uintptr_t)address >= bookkeep_end && (uintptr_t)address < HEAP_END &&
      (uintptr_t)address % 4096 == 0) {
    size_t addressHolder =
        ((uintptr_t)address - (uintptr_t)HEAP_START) / PGSIZE;
    uint8_t *heap = (uint8_t *)HEAP_START;
